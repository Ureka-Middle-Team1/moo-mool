...
| <img src="https://github.com/user-attachments/assets/936460d7-b497-4be1-af16-c5be2657bd05" width="220"/> | **3. 🧠 AI 챗봇 요금제 추천 (텍스트/음성)**<br/><br/>- `GPT‑4.1‑nano` 기반 FSM 구조로 questionId에 따라 질문 흐름 및 상태 전이 제어<br/>- 응답은 프롬프트 기반 정규화 과정을 거쳐 Smart Choice API 입력 포맷에 맞게 가공<br/>- 정확 추천(FSM) ↔ 자연스러운 추천(프롬프트) 모드 전환이 가능하도록 설계<br/>- 감정 표현, 반말 등 자연어 응답 흐름은 `recentMessages`로 분기 처리<br/>- 수집된 항목 상태는 `lastSummary`를 기준으로 판단해 반복 질문 최소화<br/>- 대화 중 이탈 후 재접속 시에도 이전 상태를 기억해 흐름을 이어감 |
