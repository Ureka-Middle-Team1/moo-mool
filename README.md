...
| <img src="https://github.com/user-attachments/assets/936460d7-b497-4be1-af16-c5be2657bd05" width="220"/> | **3. 🧠 AI 챗봇 요금제 추천 (텍스트/음성)**<br/><br/>- 정확 추천(FSM) ↔ 자연스러운 추천(프롬프트) 모드 전환이 가능하도록 설계<br/>- `GPT‑4.1‑nano`를 FSM 모드의 응답 정규화에, `GPT‑4.1‑mini`를 자연 모드의 맥락 이해에 활용하여 모델 목적에 따라 분리 적용<br/>- FSM 구조로 `questionId` 기반 질문 흐름과 상태 전이를 제어하며, 응답은 정형화되어 Smart Choice API 포맷에 맞게 가공<br/>- 감정 표현, 반말 등 자연어 응답 흐름은 `recentMessages`의 대화 내역을 통해 분석<br/>- 수집된 항목 상태는 `lastSummary`를 기준으로 판단해 반복 질문 최소화<br/>- 대화 중 이탈 후 재접속 시에도 이전 상태를 기억해 흐름을 이어감 |
